...
cost-effectiveness. Four results follow. First, the path of least
...
Purdue level~2 or below, and the corpus agrees: 55\% of incidents caused an availability impact, while
only 10\% are reported to have interacted with equipment below level~3. Second, recovery capability dominates prevention in
...
objective. Fourth, measured against four baseline prioritisations on the same corpus, the
model places the median observed incident at the $4.8$th percentile of the
routes to its consequence, where depth-into-OT and zone-crossing heuristics
place it at the $95.9$th and $98.9$th; a ranking that ignores detection entirely, however, performs
indistinguishably, so the containment term earns its place in the loss
estimate and not in the ranking. The model, corpus, experiments and figures are released as a
