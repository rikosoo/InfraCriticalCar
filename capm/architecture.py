...
EDGES: List[E] = [
    ...
    # --- Cloud and SaaS --------------------------------------------------
    ("cld_tenant", "cld_storage", "T1530", 0.75, 0.45, 4, ("C16",), "Control-plane access lists and reads the tenant's own buckets", ("INC-10",)),
    ("cld_tenant", "cld_cicd", "T1078.004", 0.60, 0.45, 8, ("C11", "C03"), "Pipeline identities are administered from the same control plane", ()),
    ...
    ("cld_tenant", "imp_ip", "T1213", 0.60, 0.50, 12, ("C16",), "Design and process data held in cloud repositories is readable once the tenant is held", ("INC-05",)),
    ("cld_storage", "imp_pii", "T1530", 0.90, 0.60, 2, ("C16",), "Reading an exposed store is the whole of the attack once it is found", ("INC-09", "INC-10")),
    ("cld_cv_backend", "imp_pii", "T1530", 0.70, 0.50, 8, ("C16", "C11"), "Telemetry back ends hold vehicle and owner records by design", ("INC-09",)),
    ("saas_crm", "imp_pii", "T1567.002", 0.80, 0.50, 6, ("C11",), "Customer records are the platform's purpose; bulk export is a supported feature", ("INC-17",)),
    ("saas_dms", "imp_pii", "T1567.002", 0.70, 0.50, 8, ("C11",), "Retail platforms hold buyer and finance records across many dealers", ("INC-11",)),
    ("saas_dms", "imp_prod_stop", "T1489", 0.35, 0.40, 8, ("C11", "C14"),
     "Retail/logistics platform outage propagating upstream", ("INC-11",)),
    # --- Collection in IT -------------------------------------------------
    ("it_erp", "it_fileshare", "T1039", 0.70, 0.40, 4, ("C07",), "ERP service accounts are broadly authorised on file shares", ()),
    ("it_user_ws", "it_fileshare", "T1039", 0.55, 0.40, 4, ("C07",), "An employee endpoint already carries the user's share entitlements",
     ("INC-07", "INC-12", "INC-13")),
    ("it_fileshare", "imp_ip", "T1567.002", 0.80, 0.50, 8, ("C07", "C16"), "Engineering data on shares is exfiltrated with ordinary user rights", ("INC-05", "INC-07")),
    # --- Supply chain into the OEM ---------------------------------------
    ("sup_tier1", "sup_edi", "T1078", 0.75, 0.40, 4, ("C14",), "The supplier's own systems are authorised on the interface by construction",
     ("INC-04", "INC-14", "INC-16", "INC-20")),
    ("sup_tier1", "imp_ip", "T1567.002", 0.55, 0.55, 12, ("C14",), "Suppliers hold OEM drawings and specifications under contract", ("INC-05", "INC-15")),
    ...
    ("sup_vendor_ra", "dmz_broker", "T1133", 0.60, 0.40, 6, ("C04",), "Vendor accounts are provisioned on the broker for maintenance", ()),
    ("sup_vendor_ra", "ops_ews", "T0822", 0.30, 0.60, 12, ("C04", "C13"),
     "Unmanaged cellular or modem access bypassing the IDMZ", ()),
    ("sup_sw_update", "ops_ews", "T0873", 0.50, 0.55, 24, ("C09", "C06"), "Engineering projects and firmware arrive from vendors as trusted files", ()),
    ("sup_usb", "ops_ews", "T0847", 0.50, 0.55, 8, ("C06", "C09"), "Field service still moves projects on removable media in many plants", ()),
    ("it_insider", "ops_ews", "T1078", 0.60, 0.50, 8, ("C13", "C06"), "A recruited insider with plant access needs no exploit", ("INC-08",)),
    ("it_insider", "it_sccm", "T1072", 0.25, 0.45, 16, ("C03", "C13"), "Deployment tooling is administered by a small group, which limits insider reach", ("INC-08",)),
    ...
    ("it_user_ws", "dmz_jump", "T1021.001", 0.35, 0.45, 8, ("C05", "C01"), "Reaching the jump server requires credentials the endpoint may not hold", ()),
    ("it_erp", "dmz_mirror", "T1210", 0.45, 0.40, 8, ("C05",), "The data broker accepts connections from the ERP side by design", ()),
    ("dmz_jump", "ops_ews", "T1021.001", 0.70, 0.40, 4, ("C05", "C03"), "The jump server exists to reach engineering hosts", ()),
    ("dmz_broker", "ops_ews", "T0822", 0.65, 0.45, 6, ("C04", "C13"), "Brokered vendor sessions terminate on the engineering workstation", ()),
    ("dmz_relay", "ops_historian", "T1072", 0.50, 0.45, 10, ("C09", "C13"), "The relay pushes packages into site operations on a schedule", ()),
    ("dmz_mirror", "ops_historian", "T0885", 0.50, 0.45, 8, ("C05", "C13"), "Mirror and historian exchange process data over a permitted port", ()),
    ("ops_ot_ad", "ops_ews", "T1078", 0.75, 0.40, 4, ("C03", "C13"), "Plant hosts are domain members of the OT directory", ()),
    ("ops_ot_ad", "sca_server", "T1078", 0.70, 0.40, 6, ("C03", "C13"), "Supervisory servers authenticate against the same OT directory", ()),
    ("ops_historian", "ops_mes", "T1210", 0.45, 0.45, 10, ("C13", "C10"), "Historian and MES share interfaces and often a maintenance account", ()),
    ("ops_mes", "sca_server", "T0886", 0.55, 0.45, 8, ("C05", "C13"), "Sequencing traffic between MES and line control is permitted", ()),
    ...
    ("ops_ews", "ctl_plc_body", "T0843", 0.70, 0.45, 6, ("C09", "C13"), "Downloading a program is the engineering workstation's normal function", ()),
    ("ops_ews", "ctl_robot", "T0843", 0.60, 0.45, 8, ("C09", "C13"), "Robot controllers accept programs from the same engineering estate", ()),
    ("ops_ews", "ctl_safety", "T0889", 0.32, 0.50, 16, ("C09", "C13"), "Safety controllers add key switches, signed projects and separate approval", ()),
    ("ops_ews", "sca_server", "T0886", 0.60, 0.40, 6, ("C13", "C05"), "Engineering hosts administer the supervisory layer", ()),
    ("sca_server", "sca_hmi", "T0886", 0.75, 0.40, 4, ("C13",), "HMIs are served and administered by the supervisory server", ()),
    ("sca_server", "ctl_plc_body", "T0855", 0.70, 0.45, 4, ("C09", "C13"), "Supervisory servers issue commands to controllers as a matter of course", ()),
    ("sca_hmi", "ctl_robot", "T0855", 0.60, 0.45, 6, ("C09", "C13"), "Operator panels command cell controllers within their area", ()),
    ("sca_server", "imp_prod_stop", "T0813", 0.60, 0.40, 4, ("C13", "C08"),
     "Denial of control at the supervisory layer", ("INC-03",)),
    ("ctl_plc_body", "prc_line", "T0831", 0.85, 0.50, 2, ("C09",), "A held controller drives the process it controls", ()),
    ("ctl_robot", "prc_line", "T0831", 0.80, 0.50, 2, ("C09",), "Cell controllers move the equipment they command", ()),
    ("ctl_robot", "prc_paint", "T0836", 0.60, 0.55, 6, ("C09", "C13"), "Parameter drift in coating is possible but constrained by recipe checks", ()),
    ("ctl_safety", "imp_safety", "T0880", 0.45, 0.50, 4, ("C09",), "A defeated safety function is a loss of protection, not automatically an injury", ()),
    ("ctl_safety", "imp_prod_stop", "T0816", 0.60, 0.40, 4, ("C09",),
     "Safety trip or controller shutdown stopping the line", ()),
    ("prc_line", "imp_prod_stop", "T0828", 0.90, 0.30, 2, (), "Manipulated line equipment stops production almost immediately", ("INC-01", "INC-04", "INC-18")),
